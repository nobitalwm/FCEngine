...
SharePtr<CAnimation> CEditAnimationDialog::GetAnimation()
{
    return m_pAnimation;
}
...
void CEditAnimationDialog::LanguageSwitch()
{
    ...
    m_pSkeSelectButton->SetLabel(CLanguageManager::GetInstance()->GetText(m_bIsSelectAll ? eL_DeselectAll : eL_SelectAll));
}
...
wxTreeItemId CEditAnimationDialog::AddModelToTree( TString ModelName )
{
    ...
    m_ModelNameMap[item] = CRenderObjectManager::GetInstance()->CreateModel();
    return item;
}
